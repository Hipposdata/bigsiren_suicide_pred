    ...
    if model is not None:
        ...
        try:
            with st.spinner('SHAP 분석 중입니다...'):
                ...
                tab1, tab2, tab3, tab4 = st.tabs(["📊 SHAP Force Plot", "📈 Feature Impact", "🎯 Waterfall", "📋 Summary"])
                with tab1:
                ...
                with tab3:
                    ...
                    try:
                        ...
                        x_pos = range(len(feature_names) + 2)
                        labels = ['Base'] + feature_names + ['Prediction']
                        for i, (label, shap_val) in enumerate(zip(feature_names, shap_vals)):
                            if shap_val >= 0:
                                ax.bar(i+1, shap_val, bottom=positions[i], color='red', alpha=0.7)
                            else:
                                ax.bar(i+1, abs(shap_val), bottom=positions[i+1], color='blue', alpha=0.7)
                        ax.bar(0, base_value, color='gray', alpha=0.7)
                        ax.bar(len(feature_names)+1, positions[-1], color='green', alpha=0.7)
                        ax.set_xticks(x_pos)
                        ax.set_xticklabels(labels, rotation=45, ha='right')
                        ax.set_ylabel('예측값')
                        ax.set_title('SHAP Waterfall Plot - 예측 과정')
                        ax.grid(True, alpha=0.3)
                        plt.tight_layout()
                        st.pyplot(fig_waterfall)
                    except Exception as e:
                        st.warning(f"Waterfall plot 생성 중 오류: {e}")
                with tab4:
